...
Written using the bottle library, it resides in the file ``web_server.py''. The
web server also uses the libgreen library extensively, to serve its clients. It
is not directly invoked from the file, rather the contents of ``web_server.py''
is imported into ``netdog_server.py'' and started as a separate thread.
